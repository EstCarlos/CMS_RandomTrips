...
function KPIBar() {
  ...
  const pagados      = BOOKINGS.filter(b => b.estado === "pagado_completo").length;
  const depositos    = BOOKINGS.filter(b => b.estado === "deposito_pagado").length;
  const vencidos     = BOOKINGS.filter(b => b.estado === "saldo_vencido").length;
  const totalSaldo   = BOOKINGS.reduce((s, b) => s + b.saldo_pendiente, 0);
...
function buildDetalleReserva(bkId: string) {
  ...
  const link = PAYMENT_LINKS.find(l => l.booking_id === bk.id);
  return {
    id: bk.id,
    cliente: cust.nombre,
    email: cust.email,
    ...
    pais: cust.pais,
    tour: tour.titulo_es,
    destino: tour.destinos_nombres.join(", "),
    fechaTour: bk.fecha_display,
    fechaCreacion: "Jun 2026",
    pax: bk.pax_total,
    total: bk.precio_total,
    pagado: bk.deposito_pagado,
    status: bk.estado === "pagado_completo" ? "confirmed" : bk.estado === "saldo_vencido" ? "pending" : "partial",
    operador: "Random Trips",
    notas: link ? `Link de pago: ${link.invoice_id} · Vence ${link.expira} · Estado: ${link.estado}` : "",
  };
}
...
export function Reservas() {
  ...
  const filtered = BOOKINGS.filter(bk => {
    ...
    const mSearch = !q || bk.id.toLowerCase().includes(q) || cust.nombre.toLowerCase().includes(q) || cust.email.toLowerCase().includes(q);
    const mStatus = !filters.status || bk.estado === filters.status;
    const mTour   = !filters.tour   || bk.tour_id === filters.tour;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20, fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <FilterBar
          filters={[
            ...
            { key: "tour",   label: "Tour",   type: "select", options: TOURS_DATA.map(t => ({ value: t.id, label: t.titulo_es })) },
          ]}
          values={filters}
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid #E5E7EB", borderRadius: 8, overflow: "hidden" }}>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={bk.id}
                    ...
                    style={{ borderBottom: i < filtered.length - 1 ? "1px solid #F1F5F9" : "none", background: isSel ? "#F0F7FF" : bk.estado === "saldo_vencido" ? "#FFFBEB" : "#FFFFFF", cursor: "pointer", transition: "background 0.08s" }}
                    onMouseEnter={e => { if (!isSel) (e.currentTarget as HTMLElement).style.background = "#F7F8FA"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = isSel ? "#F0F7FF" : bk.estado === "saldo_vencido" ? "#FFFBEB" : "#FFFFFF"; }}
                  >
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: "#0F172A" }}>{cust.nombre}</div>
                      <div style={{ fontSize: 11, color: "#94A3B8" }}>{cust.email}</div>
                    </td>
                    <td style={{ padding: "12px 14px", fontSize: 12, color: "#475569", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tour.titulo_es}</td>
                    <td style={{ padding: "12px 14px", fontSize: 12, color: "#475569", whiteSpace: "nowrap" }}>{bk.fecha_display}</td>
                    <td style={{ padding: "12px 14px", textAlign: "center", fontWeight: 600 }}>{bk.pax_total}</td>
                    <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: 700, fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>{formatDOP(bk.precio_total)}</td>
                    <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: 600, color: "#16A34A", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>{formatDOP(bk.deposito_pagado)}</td>
                    <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: 600, color: bk.saldo_pendiente > 0 ? "#F13540" : "#94A3B8", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        ...
                        {link && (
                          <span style={{ fontSize: 10, color: link.estado === "vencido" ? "#F13540" : "#94A3B8" }}>
                            {link.estado === "vencido" ? "⚠️ Link vencido" : link.estado === "enviado" ? "📤 Link enviado" : "🕐 Sin link"}
                          </span>
                        )}
                      </div>
                    </td>
        ...
        <div style={{ padding: "12px 20px", borderTop: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", fontSize: 12, color: "#475569" }}>
          ...
          <span style={{ color: "#94A3B8" }}>WhatsApp atención: {SITE_CONFIG.contacto.whatsapp}</span>
        </div>
      </div>
    </div>
  );
